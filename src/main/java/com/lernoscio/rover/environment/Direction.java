package com.lernoscio.rover.environment;

/**
 *  An enum that represents direction and four
 *  possible direction values. Each direction is
 *  aware of the direction on it's left and right.
 *  Each direction is also aware of unit step
 *  distance in/on a room/stair for x coordinate
 *  and y coordinate.
 */

public enum Direction {

    N(0,1) {
        @Override
        public Direction left() {
            return W;
        }

        @Override
        public Direction right() {
            return E;
        }

        @Override
        public Direction turn() {
            return S;
        }
    },

    S(0,-1) {
        @Override
        public Direction right() {
            return W;
        }

        @Override
        public Direction left() {
            return E;
        }

        @Override
        public Direction turn() {
            return N;
        }
    },

    E(1,0) {
        @Override
        public Direction right() {
            return S;
        }

        @Override
        public Direction left() {
            return N;
        }

        @Override
        public Direction turn() {
            return W;
        }
    },

    W(-1,0) {
        @Override
        public Direction right() {
            return N;
        }

        @Override
        public Direction left() {
            return S;
        }

        @Override
        public Direction turn() {
            return E;
        }
    };

    private final int stepSizeOnXAxis;
    private final int stepSizeOnYAxis;

    Direction(final int stepSizeOnXAxis, final int stepSizeOnYAxis) {
        this.stepSizeOnXAxis = stepSizeOnXAxis;
        this.stepSizeOnYAxis = stepSizeOnYAxis;
    }

    public abstract Direction right();
    public abstract Direction left();
    public abstract Direction turn();

    public int stepSizeForXAxis() {
        return this.stepSizeOnXAxis;
    }

    public int stepSizeForYAxis() {
        return this.stepSizeOnYAxis;
    }

}